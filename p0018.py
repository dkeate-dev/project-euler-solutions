...
for line in triangle:
  new_triangle.append(line.split(" "))

for i in range(len(new_triangle)):
  for j in range(len(new_triangle[i])):
    new_triangle[i][j] = int(new_triangle[i][j])
...
for i in range(len(new_triangle)-2, -1, -1):
  for j in range(len(new_triangle[i])):
    new_triangle[i][j] += maximum(new_triangle[i+1][j], new_triangle[i+1][j+1])
